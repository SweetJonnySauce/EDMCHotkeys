...
| `EDMC_HOTKEYS_ENABLE_WINDOWS_LOW_LEVEL_HOOK` | `0` (disabled) | `edmc_hotkeys/backends/windows.py` | Enables Windows low-level keyboard hook fallback for side-specific modifier matching. | Medium: low-level hooks can increase lifecycle/error-path complexity. | Remove when Windows baseline routing supports intended side-specific behavior without experimental fallback gating. |
